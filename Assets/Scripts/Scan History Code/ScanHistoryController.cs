...
using TMPro;
/// <summary>
/// * This Class Controls overall behavior of Scan History Page
/// * This is a singleton
/// * Attached to ScanHistoryBack
/// </summary>
public class ScanHistoryController : MonoBehaviour {
    public bool rollable { get; set; }
    private static ScanHistoryController instance;
    public static ScanHistoryController Instance { get { return instance; } }
    public GameObject DetailPage;
    public GameObject ProductName;
    public GameObject ProductDate;
    public GameObject ProductLocation;
    public GameObject SearchBar;
    public Button SearchButton; // opens and closes search bar
    public Button All;
    ...
    public void PopulateContainers() {
        ...
        while (PC.CurDic.Count > i) {
            ...
            go.GetComponent<FoodEntryContainer>().PIUpdate(PC.CurDic[PC.CurDic.Count - i - 1]);
            i++;
        }
    ...
    /// <summary>
    /// Filters a new category by what is typed into the search bar
    /// </summary>
    public void FilterCategory() {
        if (TextInput.text.Length > 1) {
            Debug.Log(TextInput.text.ToLower());
            PC.CurDic = new List<ProductInfo>();
            foreach(ProductInfo pi in PC.products) {
                Debug.Log(pi.Name.ToLower());
                if (pi.Name.ToLower().Contains(TextInput.text.ToLower())) {
    ...
    /// * Adjust the FoodEntryContainer Container's position accordingly
    /// </summary>
        ...
        foreach (GameObject go in Containers) {
            ...
            /*if (curPos.y > containerHeight) {
                ...
                try {
                    ...
                    go.GetComponent<v>().PIUpdate(pc.GetProduct(i, currentCate));
                }
            ...
            /*if (curPos.y < (-Containers.Count + microAdjustVal) * containerHeight) {
                ...
                try {
                    ...
                    go.GetComponent<FoodEntryContainer>().PIUpdate(pc.GetProduct(i, currentCate));
                }
                catch (System.Exception ex) {
                    Debug.Log(totalDisRollingDis);
                    Debug.Log(ex.StackTrace);
    ...
    public void ToggleSearchBar() {
        ...
        Vector3 allTransform = All.GetComponent<RectTransform>().transform.position;
        Vector3 containsAddedSugarTransform = ContainsAddedSugar.GetComponent<RectTransform>().transform.position;
        Vector3 noAddedSugarTransform = NoAddedSugar.GetComponent<RectTransform>().transform.position;
        int moveAmount = 180;
        SearchBar.SetActive(!SearchBar.activeSelf);
        //if (SearchBar.GetComponent<RectTransform>().transform.localScale.x == 1) {
        // Close
        //SearchBar.GetComponent<RectTransform>().transform.localScale = new Vector3(0, 1, 1);
        /*            All.GetComponent<RectTransform>().transform.position = new Vector3(allTransform.x - moveAmount, allTransform.y, allTransform.z);
                    ContainsAddedSugar.GetComponent<RectTransform>().transform.position = new Vector3(containsAddedSugarTransform.x - moveAmount, containsAddedSugarTransform.y, containsAddedSugarTransform.z);
                    NoAddedSugar.GetComponent<RectTransform>().transform.position = new Vector3(noAddedSugarTransform.x - moveAmount, noAddedSugarTransform.y, noAddedSugarTransform.z);*/
        //}
        //else {
        // Open
        //SearchBar.GetComponent<RectTransform>().transform.localScale = new Vector3(1, 1, 1);
        /*            All.GetComponent<RectTransform>().transform.position = new Vector3(allTransform.x + moveAmount, allTransform.y, allTransform.z);
                    ContainsAddedSugar.GetComponent<RectTransform>().transform.position = new Vector3(containsAddedSugarTransform.x + moveAmount, containsAddedSugarTransform.y, containsAddedSugarTransform.z);
                    NoAddedSugar.GetComponent<RectTransform>().transform.position = new Vector3(noAddedSugarTransform.x + moveAmount, noAddedSugarTransform.y, noAddedSugarTransform.z);*/
        //}
    ...
    private void SetHighlights(Category cate) {
        if (cate == Category.all) {
            ...
            All.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Bold; // set "all" button to selected
            NoAddedSugar.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Normal;
            ContainsAddedSugar.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Normal;
        }
        else if (cate == Category.noaddedsugar) {
            //background.GetComponentInChildren<Image>().sprite = Backgrounds[1];
            All.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Normal;
            NoAddedSugar.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Bold; // set "No Sugar Added" to selected
            ContainsAddedSugar.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Normal;
        }
        else {
            ...
            All.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Normal;
            NoAddedSugar.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Normal;
            ContainsAddedSugar.GetComponentInChildren<TextMeshProUGUI>().fontStyle = FontStyles.Bold; // set "Contains Sugar Added" to selected
        }
    }
    /// <summary>
    ...
    /// <summary>
    /// * Reset the Container's Position every time user Open FoodEntryContainer
